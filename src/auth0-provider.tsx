...
export interface Auth0ProviderOptions {
   ...
   * It uses `window.history` but you might want to overwrite this if you are using a custom router, like `solid-router`
   ...
   * `'example.eu.auth0.com'` or , `'example.company.com'`
   * (when using [custom domains](https://auth0.com/docs/custom-domains))
   */
  domain: string
  /**
   * The issuer to be used for validation of JWT, optionally defaults to the domain above
   */
  issuer?: string
  /**
   ...
   * the user must be reauthenticate.
   */
 ...
 */
export default (props: Auth0ProviderOptions): JSX.Element => {
  const merged = mergeProps({ onRedirectCallback: defaultOnRedirectCallback }, props)
  ...
  const client = new Auth0Client(toAuth0ClientOptions(clientOpts as Auth0ProviderOptions))
  const { state, dispatch } = useReducer(initialAuthState)
  ...
  const buildAuthorizeUrl = (opts?: RedirectLoginOptions): Promise<string> =>
    client.buildAuthorizeUrl(toAuth0LoginRedirectOptions(opts))

  const buildLogoutUrl = (opts?: LogoutUrlOptions): string => client.buildLogoutUrl(opts)

  const loginWithRedirect = (opts?: RedirectLoginOptions): Promise<void> =>
  ...
  const logout = (opts: LogoutOptions = {}): Promise<void> | void => {
  ...
  const getAccessTokenSilently = // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const getIdTokenClaims = (opts?: GetIdTokenClaimsOptions) => client.getIdTokenClaims(opts)
  ...
  return <Auth0Context.Provider value={contextValue}>{local.children}</Auth0Context.Provider>
}
